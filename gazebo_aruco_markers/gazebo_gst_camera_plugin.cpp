...
void GstCameraPlugin::startGstThread() {
  ...
  if (useCuda) {
    ...
    g_object_set(G_OBJECT(encoder), "bitrate", 2048, "preset", 1, nullptr); //800
  } else {
    encoder = gst_element_factory_make("x264enc", nullptr);
    g_object_set(G_OBJECT(encoder), "bitrate", 2048, "speed-preset", 6, "tune", 4, "key-int-max", 10, nullptr); //800
  }
